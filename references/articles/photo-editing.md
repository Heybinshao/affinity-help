---
title: "Photo editing - Affinity Help Center"
source: https://www.affinity.studio/help/photo-editing/
slug: photo-editing
fetched: 2026-08-06
---

# Photo editing - Affinity Help Center

> 官方来源：https://www.affinity.studio/help/photo-editing/

[Skip to main content](https://www.affinity.studio/help/photo-editing/#_r_0_)

[![Image 1](https://content-management-files.canva.com/78303f82-4688-4614-99c7-ad7970d208a9/affinity_logotype_white_min.svg)](https://www.affinity.studio/)

*   Product    

*   Discover    

*   [Help](https://www.affinity.studio/help)

[Get Affinity](https://www.affinity.studio/get-affinity)

[![Image 2](https://content-management-files.canva.com/b916d6a5-3b78-490b-a56a-673b7dc606d0/affinity_symbol_white_min.svg)](https://www.affinity.studio/)

Submit Search

1.   [Help Center](https://www.affinity.studio/help/)
2.   Photo editing

# Photo editing

Edit, crop, retouch, and work with images and pixels

* * *

### Sizing, cropping, and warping

*   [Changing image size](https://www.affinity.studio/help/size-transform-image-size/)
*   [Changing canvas size](https://www.affinity.studio/help/size-transform-canvas-size/)
*   [Pixel art resizing](https://www.affinity.studio/help/size-transform-pixelart/)
*   [Cropping and straightening images](https://www.affinity.studio/help/size-transform-cropping/)
*   [Cropping objects and groups](https://www.affinity.studio/help/size-transform-cropping-objects-groups/)

[See all](https://www.affinity.studio/help/photo-editing/#)

* * *

### Filters

*   [All filters](https://www.affinity.studio/help/filters-all-filters/)
*   [Auto filters](https://www.affinity.studio/help/filters-filter-auto/)
*   [Applying filters](https://www.affinity.studio/help/filters-filters-applying/)
*   [Astrophotography filters](https://www.affinity.studio/help/filters-astro-filters/)
*   [Blur filters](https://www.affinity.studio/help/filters-blur-filters/)

[See all](https://www.affinity.studio/help/photo-editing/#)

* * *

### Photo Editing Concepts

*   [Understanding tonality](https://www.affinity.studio/help/understanding-tonality/)
*   [Recovering and preserving highlights](https://www.affinity.studio/help/recovering-preserving-highlights/)
*   [Editing tonality in Develop Studio (RAW)](https://www.affinity.studio/help/editing-tonality-develop-studio-raw/)
*   [Global tonal adjustments](https://www.affinity.studio/help/global-tonal-adjustments/)
*   [Editing tonality using blend options](https://www.affinity.studio/help/edit-tonality-blend-options/)

[See all](https://www.affinity.studio/help/photo-editing/#)

* * *

### Develop Studio

*   [Developing RAW images](https://www.affinity.studio/help/raw-raw/)
*   [Using masks](https://www.affinity.studio/help/raw-using-masks-develop-studio/)
*   [Lens correction](https://www.affinity.studio/help/raw-raw-lenscorrection/)
*   [Focus Peaking](https://www.affinity.studio/help/focus-peaking/)

* * *

### Liquify Studio

*   [Warping using the Liquify Studio](https://www.affinity.studio/help/liquify-persona-liquify/)
*   [Masking in the Liquify Studio](https://www.affinity.studio/help/liquify-persona-liquify-masking/)

* * *

### Tone Mapping Studio

*   [32-bit HDR editing](https://www.affinity.studio/help/hdr-hdr-editing/)
*   [Merging to 32-bit HDR](https://www.affinity.studio/help/hdr-hdr-merging/)
*   [Tone mapping HDR images](https://www.affinity.studio/help/hdr-hdr-tonemapping/)
*   [32-bit OpenEXR support](https://www.affinity.studio/help/hdr-openexr/)

* * *

### Pixel selection

*   [Creating pixel selections](https://www.affinity.studio/help/selections-selections-create/)
*   [Painting pixel selections](https://www.affinity.studio/help/selections-selections-brush/)
*   [Flooding pixel selections](https://www.affinity.studio/help/selections-selections-flood/)
*   [Drawing pixel selections](https://www.affinity.studio/help/selections-selections-freehand/)
*   [Range pixel selections](https://www.affinity.studio/help/selections-selections-range/)

[See all](https://www.affinity.studio/help/photo-editing/#)

* * *

### Raster painting

*   [Painting raster brush strokes](https://www.affinity.studio/help/painting-raster-painting/)
*   [Modifying raster brushes](https://www.affinity.studio/help/painting-raster-modify/)
*   [Symmetry and Mirror](https://www.affinity.studio/help/painting-symmetry-brushes/)
*   [Creating custom raster brushes](https://www.affinity.studio/help/painting-custom-raster-brushes/)
*   [Multi-brushes](https://www.affinity.studio/help/painting-raster-multi-brushes/)

[See all](https://www.affinity.studio/help/photo-editing/#)

* * *

### Retouching

*   [Retouching](https://www.affinity.studio/help/retouching-retouch/)
*   [Removing red eye](https://www.affinity.studio/help/retouching-retouching-red-eye/)
*   [Removing blemishes](https://www.affinity.studio/help/retouching-retouching-blemishes/)
*   [Cloning and healing](https://www.affinity.studio/help/retouching-retouching-cloning-healing/)
*   [Inpainting](https://www.affinity.studio/help/retouching-retouching-inpainting/)

[See all](https://www.affinity.studio/help/photo-editing/#)

* * *

### Channels

*   [Using channels](https://www.affinity.studio/help/channels-using-channels/)
*   [Editing channels](https://www.affinity.studio/help/channels-channels-selecting-editing/)
*   [Spare channels](https://www.affinity.studio/help/channels-spare-channels/)
*   [Masking from channels](https://www.affinity.studio/help/channels-masking-channels/)

* * *

### Macros and batch jobs

*   [Macros](https://www.affinity.studio/help/macros-batch-macros/)
*   [Batch jobs](https://www.affinity.studio/help/macros-batch-batchjobs/)

* * *

### Image processing

*   [Stitching Panoramas](https://www.affinity.studio/help/panorama-panorama-stitching/)
*   [Editing Panoramas](https://www.affinity.studio/help/panorama-panorama-editing/)
*   [Image stacks](https://www.affinity.studio/help/stacking-stacks/)
*   [Exposure merging using stacks](https://www.affinity.studio/help/stacking-stacks-exposuremerge/)
*   [Object removal using stacks](https://www.affinity.studio/help/stacking-stacks-objectremoval/)

[See all](https://www.affinity.studio/help/photo-editing/#)

* * *

### Advanced photo editing

*   [Linking](https://www.affinity.studio/help/object-control-linking/)
*   [Compound layer masks](https://www.affinity.studio/help/layers-compound-masks/)
*   [Live layer masks](https://www.affinity.studio/help/live-masks-live-layer-masks/)
*   [Fade](https://www.affinity.studio/help/object-control-fade/)
*   [Scanning images (Pixel Studio, Mac only)](https://www.affinity.studio/help/media-scanning-images/)

[See all](https://www.affinity.studio/help/photo-editing/#)

* * *

### Photo editing tools

*   [Adjustment Brush Tool](https://www.affinity.studio/help/tools-tools-adjustment-brush/)
*   [Background Erase Brush Tool](https://www.affinity.studio/help/tools-tools-background-erase-brush/)
*   [Blemish Removal Tool](https://www.affinity.studio/help/tools-tools-blemish-removal/)
*   [Blur Brush Tool](https://www.affinity.studio/help/tools-tools-blur-brush/)
*   [Burn Brush Tool](https://www.affinity.studio/help/tools-tools-burn-brush/)

[See all](https://www.affinity.studio/help/photo-editing/#)

* * *

### Astrophotography

*   [About astrophotography stacking](https://www.affinity.studio/help/astrophotography-astro-about/)
*   [Creating an astrophotography stack](https://www.affinity.studio/help/astrophotography-astro-creating/)
*   [Files panel](https://www.affinity.studio/help/panels-astro-panel-files/)
*   [RAW Options panel](https://www.affinity.studio/help/panels-astro-panel-raw-options/)
*   [Stacking Options panel](https://www.affinity.studio/help/panels-astro-panel-stacking-options/)

[See all](https://www.affinity.studio/help/photo-editing/#)

*   
    *   Product

    *   [Get Affinity](https://www.affinity.studio/get-affinity)
    *   [Graphic design](https://www.affinity.studio/graphic-design-software)
    *   [Photo editing](https://www.affinity.studio/photo-editing-software)
    *   [Page layout](https://www.affinity.studio/page-layout-software)
    *   [Canva integrations](https://www.affinity.studio/canva-integrations)
    *   [Product integrations](https://www.affinity.studio/integrations)

*   
    *   Discover

    *   [Inspired.af](https://www.affinity.studio/blog)
    *   [Spotlight](https://www.affinity.studio/spotlight)
    *   [Learning resources](https://www.affinity.studio/resources)
    *   [Press](https://www.canva.com/newsroom/news/all-new-affinity)

*   
    *   Help

    *   [Help Center](https://www.affinity.studio/help)
    *   [Affinity V2](https://affinity.serif.com/v2/)
    *   [Affinity V2 account](https://store.serif.com/sign-in/)
    *   [April 2026 update](https://www.affinity.studio/blog/affinity-update-april-2026)
    *   [Release notes](https://www.affinity.studio/help/release-notes/)

*   
    *   Connect

    *   [About](https://www.affinity.studio/about)
    *   [Careers](https://www.lifeatcanva.com/)
    *   [Contact Sales](https://www.canva.com/contact-sales/affinity/)

*   Product 

    *   [Get Affinity](https://www.affinity.studio/get-affinity)
    *   [Graphic design](https://www.affinity.studio/graphic-design-software)
    *   [Photo editing](https://www.affinity.studio/photo-editing-software)
    *   [Page layout](https://www.affinity.studio/page-layout-software)
    *   [Canva integrations](https://www.affinity.studio/canva-integrations)
    *   [Product integrations](https://www.affinity.studio/integrations)

*   Discover 

    *   [Inspired.af](https://www.affinity.studio/blog)
    *   [Spotlight](https://www.affinity.studio/spotlight)
    *   [Learning resources](https://www.affinity.studio/resources)
    *   [Press](https://www.canva.com/newsroom/news/all-new-affinity)

*   Help 

    *   [Help Center](https://www.affinity.studio/help)
    *   [Affinity V2](https://affinity.serif.com/v2/)
    *   [Affinity V2 account](https://store.serif.com/sign-in/)
    *   [April 2026 update](https://www.affinity.studio/blog/affinity-update-april-2026)
    *   [Release notes](https://www.affinity.studio/help/release-notes/)

*   Connect 

    *   [About](https://www.affinity.studio/about)
    *   [Careers](https://www.lifeatcanva.com/)
    *   [Contact Sales](https://www.canva.com/contact-sales/affinity/)

English (US)

*   [![Image 3: Instagram](https://content-management-files.canva.com/42a74cd5-ccc1-43f4-847b-cfee15b387d6/social_instagram.svg)](https://www.instagram.com/affinity/)
*   [![Image 4: Facebook](https://content-management-files.canva.com/c0ee217b-1e52-423a-95cb-6674227da28c/social_facebook.svg)](https://www.facebook.com/affinity)
*   [![Image 5: Threads](https://content-management-files.canva.com/5a46d885-129b-44bc-9bb5-91a1cd24519a/social_threads.svg)](https://www.threads.com/@affinity)
*   [![Image 6: Discord](https://content-management-files.canva.com/4c86f6ba-0400-48aa-b33b-d1ac6e06406f/social_discord.svg)](https://affin.link/dc)
*   [![Image 7: TikTok](https://content-management-files.canva.com/c38ff714-7599-4dc5-869a-7bc1553075e5/social_tiktok.svg)](https://www.tiktok.com/@weareaffinity)
*   [![Image 8: X](https://content-management-files.canva.com/1c162bcb-f637-4d23-ac3f-909ebf33f8c0/social_twitter.svg)](https://x.com/Affinity)
*   [![Image 9: Youtube](https://content-management-files.canva.com/719b9b23-2ce5-4dcf-8487-d81d4fc52d81/social_youtube.svg)](https://www.youtube.com/AffinitySuite)

*   [Privacy](https://www.canva.com/policies/privacy-policy/)
*   [Terms & Conditions](https://www.canva.com/policies/)
*   © 2026 All Rights Reserved, Affinity 

Show main menu

Suggestions will appear below the field as you type

Displaying 5 of 10 links. Use the See All button at the end of the list to show all topics.Displaying 5 of 13 links. Use the See All button at the end of the list to show all topics.Displaying 5 of 7 links. Use the See All button at the end of the list to show all topics.Displaying 4 of 4 links. Use the See All button at the end of the list to show all topics.Displaying 2 of 2 links. Use the See All button at the end of the list to show all topics.Displaying 4 of 4 links. Use the See All button at the end of the list to show all topics.Displaying 5 of 17 links. Use the See All button at the end of the list to show all topics.Displaying 5 of 9 links. Use the See All button at the end of the list to show all topics.Displaying 5 of 7 links. Use the See All button at the end of the list to show all topics.Displaying 4 of 4 links. Use the See All button at the end of the list to show all topics.Displaying 2 of 2 links. Use the See All button at the end of the list to show all topics.Displaying 5 of 16 links. Use the See All button at the end of the list to show all topics.Displaying 5 of 6 links. Use the See All button at the end of the list to show all topics.Displaying 5 of 45 links. Use the See All button at the end of the list to show all topics.Displaying 5 of 6 links. Use the See All button at the end of the list to show all topics.

## Affinity Cookies

Affinity is a Canva product, and Canva uses essential cookies to make this site work. We'd like to use other cookies to improve and personalise your visit, tailor ads you see from us on Affinity and partner sites, and to analyze our website's performance, but only if you accept. Learn more about your choices in the [Canva cookies policy.](https://www.canva.com/policies/cookies-policy/)

Manage Cookies Accept All

![Image 10: Affinity Logo](https://cdn-au.onetrust.com/logos/3dbea99f-abc0-4dbd-bcd7-8f6dfcaea28d/019a2649-863c-7345-9b60-9a618abba327/5c38b3e1-3c43-4eda-b904-88377c9b9c2e/full-logo.png)

## Manage Cookies

Cookies and similar technologies collect certain information about how you’re using our website. Some of them are essential, and without them this affinity.studio website might not work. But others are optional, and you get to choose whether we use them or not. 

 Hungry for more? 

[Read our full Cookie policy here.](https://www.canva.com/policies/cookies-policy/)

Allow All

#### Strictly Necessary Cookies

Always Active

These cookies are necessary for the website to function and cannot be switched off in our systems. They are usually only set in response to actions made by you which amount to a request for services, such as setting your privacy preferences, logging in or filling in forms. You can set your browser to block or alert you about these cookies, but some parts of the site will not then work.

Cookies Details

#### Performance Cookies

- [x] Performance Cookies 

These cookies allow us to count visits and traffic sources so we can measure and improve the performance of our site. They help us to know which pages are the most and least popular and see how visitors move around the site. All information these cookies collect is aggregated and therefore anonymous. If you do not allow these cookies we will not know when you have visited our site, and will not be able to monitor its performance.

Cookies Details

#### Targeting Cookies

- [x] Targeting Cookies 

These cookies may be set through our site by our advertising partners. They may be used by those companies to build a profile of your interests and show you relevant adverts on other sites. They do not store directly personal information, but are based on uniquely identifying your browser and internet device. If you do not allow these cookies, you will experience less targeted advertising.

Cookies Details

### Learn More

Clear
*   - [x] checkbox label label 

Apply Cancel

Consent Leg.Interest

- [x] checkbox label label

- [x] checkbox label label

- [x] checkbox label label

Reject All Confirm My Choices

[![Image 11: Powered by Onetrust](https://cdn-au.onetrust.com/logos/static/powered_by_logo.svg)](https://www.onetrust.com/solutions/consent-and-preferences/)
