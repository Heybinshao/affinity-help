---
title: "Design fundamentals - Affinity Help Center"
source: https://www.affinity.studio/help/design-fundamentals/
slug: design-fundamentals
fetched: 2026-08-06
---

# Design fundamentals - Affinity Help Center

> 官方来源：https://www.affinity.studio/help/design-fundamentals/

[Skip to main content](https://www.affinity.studio/help/design-fundamentals/#_r_0_)

[![Image 1](https://content-management-files.canva.com/78303f82-4688-4614-99c7-ad7970d208a9/affinity_logotype_white_min.svg)](https://www.affinity.studio/)

*   Product    

*   Discover    

*   [Help](https://www.affinity.studio/help)

[Get Affinity](https://www.affinity.studio/get-affinity)

[![Image 2](https://content-management-files.canva.com/b916d6a5-3b78-490b-a56a-673b7dc606d0/affinity_symbol_white_min.svg)](https://www.affinity.studio/)

Submit Search

1.   [Help Center](https://www.affinity.studio/help/)
2.   Design fundamentals

# Design fundamentals

Work with core design features like color, text, and objects

* * *

### Layers

*   [About layers](https://www.affinity.studio/help/layers-about-layers/)
*   [Creating layers](https://www.affinity.studio/help/layers-create-layers/)
*   [Renaming layers](https://www.affinity.studio/help/layers-rename-layers/)
*   [Layer drop zones](https://www.affinity.studio/help/layers-layer-drop-zones/)
*   [Layer opacity](https://www.affinity.studio/help/layers-layer-opacity/)

[See all](https://www.affinity.studio/help/design-fundamentals/#)

* * *

### Adjustments

*   [All adjustments](https://www.affinity.studio/help/adjustments-all-adjustments/)
*   [Applying adjustments](https://www.affinity.studio/help/adjustments-adjustment-applying/)
*   [Astrophotography adjustments](https://www.affinity.studio/help/adjustments-astro-adjustments/)
*   [Color adjustments](https://www.affinity.studio/help/adjustments-clr-adjustments/)
*   [Tonal adjustments](https://www.affinity.studio/help/adjustments-tonal-adjustments/)

[See all](https://www.affinity.studio/help/design-fundamentals/#)

* * *

### Color

*   [About color](https://www.affinity.studio/help/clr-about-clr/)
*   [Color models](https://www.affinity.studio/help/clr-clr-models/)
*   [Color spaces](https://www.affinity.studio/help/clr-clr-spaces/)
*   [Color management](https://www.affinity.studio/help/clr-clr-profiles/)
*   [Using OpenColorIO](https://www.affinity.studio/help/clr-ocio/)

[See all](https://www.affinity.studio/help/design-fundamentals/#)

* * *

### Objects

*   [Aligning](https://www.affinity.studio/help/object-control-align/)
*   [Converting to curves](https://www.affinity.studio/help/object-control-converttocurves/)
*   [Copying and pasting](https://www.affinity.studio/help/object-control-copy-paste-options/)
*   [Distributing/spacing](https://www.affinity.studio/help/object-control-distribute/)
*   [Duplicating](https://www.affinity.studio/help/object-control-duplicate/)

[See all](https://www.affinity.studio/help/design-fundamentals/#)

* * *

### Text

*   [Working with text](https://www.affinity.studio/help/text-text-general/)
*   [Artistic text](https://www.affinity.studio/help/text-art-text/)
*   [Frame text](https://www.affinity.studio/help/text-frame-text/)
*   [Text on a path](https://www.affinity.studio/help/text-path-text/)
*   [Shape text](https://www.affinity.studio/help/text-shape-text/)

[See all](https://www.affinity.studio/help/design-fundamentals/#)

* * *

### Design aids

*   [Advanced axonometric grids](https://www.affinity.studio/help/design-aids-grids-axonometric/)
*   [Baseline grids](https://www.affinity.studio/help/design-aids-baseline-grids/)
*   [Clip to Canvas](https://www.affinity.studio/help/design-aids-clip-to-canvas/)
*   [Constraints](https://www.affinity.studio/help/design-aids-constraints/)
*   [Construction snapping for curves](https://www.affinity.studio/help/design-aids-construction-snapping/)

[See all](https://www.affinity.studio/help/design-fundamentals/#)

* * *

### Shape tools

*   [Arrow Tool](https://www.affinity.studio/help/tools-tools-arrow/)
*   [Cat Tool](https://www.affinity.studio/help/tools-tools-cat/)
*   [Callout Ellipse Tool](https://www.affinity.studio/help/tools-tools-callout-ellipse/)
*   [Callout Rounded Rectangle Tool](https://www.affinity.studio/help/tools-tools-callout-rounded-rectangle/)
*   [Cloud Tool](https://www.affinity.studio/help/tools-tools-cloud/)

[See all](https://www.affinity.studio/help/design-fundamentals/#)

* * *

### Text tools

*   [Artistic Text Tool](https://www.affinity.studio/help/tools-tools-art-text/)
*   [Frame Text Tool](https://www.affinity.studio/help/tools-tools-frame-text/)
*   [Table Tool](https://www.affinity.studio/help/tools-tools-table/)

* * *

### Supported file formats

*   [Supported file formats](https://www.affinity.studio/help/appendix-fileformat/)
*   [Affinity supported RAW files](https://www.affinity.studio/help/supported-raw-files/)
*   [List of Affinity supported RAW files (camera brands from A-G)](https://www.affinity.studio/help/supported-raw-files-1/)
*   [List of Affinity supported RAW files (camera brands from H-N))](https://www.affinity.studio/help/supported-raw-files-2/)
*   [List of Affinity supported RAW files (camera brands from O-Z)](https://www.affinity.studio/help/supported-raw-files-3/)

[See all](https://www.affinity.studio/help/design-fundamentals/#)

* * *

### Panels

*   [32-bit Preview panel](https://www.affinity.studio/help/panels-32bit-panel/)
*   [Adjustment panel](https://www.affinity.studio/help/panels-adjustments-panel/)
*   [Anchors panel](https://www.affinity.studio/help/panels-anchors-panel/)
*   [Appearance panel](https://www.affinity.studio/help/panels-appearance-panel/)
*   [Ask panel](https://www.affinity.studio/help/panels-ask-ritson-panel/)

[See all](https://www.affinity.studio/help/design-fundamentals/#)

*   
    *   Product

    *   [Get Affinity](https://www.affinity.studio/get-affinity)
    *   [Graphic design](https://www.affinity.studio/graphic-design-software)
    *   [Photo editing](https://www.affinity.studio/photo-editing-software)
    *   [Page layout](https://www.affinity.studio/page-layout-software)
    *   [Canva integrations](https://www.affinity.studio/canva-integrations)
    *   [Product integrations](https://www.affinity.studio/integrations)

*   
    *   Discover

    *   [Inspired.af](https://www.affinity.studio/blog)
    *   [Spotlight](https://www.affinity.studio/spotlight)
    *   [Learning resources](https://www.affinity.studio/resources)
    *   [Press](https://www.canva.com/newsroom/news/all-new-affinity)

*   
    *   Help

    *   [Help Center](https://www.affinity.studio/help)
    *   [Affinity V2](https://affinity.serif.com/v2/)
    *   [Affinity V2 account](https://store.serif.com/sign-in/)
    *   [April 2026 update](https://www.affinity.studio/blog/affinity-update-april-2026)
    *   [Release notes](https://www.affinity.studio/help/release-notes/)

*   
    *   Connect

    *   [About](https://www.affinity.studio/about)
    *   [Careers](https://www.lifeatcanva.com/)
    *   [Contact Sales](https://www.canva.com/contact-sales/affinity/)

*   Product 

    *   [Get Affinity](https://www.affinity.studio/get-affinity)
    *   [Graphic design](https://www.affinity.studio/graphic-design-software)
    *   [Photo editing](https://www.affinity.studio/photo-editing-software)
    *   [Page layout](https://www.affinity.studio/page-layout-software)
    *   [Canva integrations](https://www.affinity.studio/canva-integrations)
    *   [Product integrations](https://www.affinity.studio/integrations)

*   Discover 

    *   [Inspired.af](https://www.affinity.studio/blog)
    *   [Spotlight](https://www.affinity.studio/spotlight)
    *   [Learning resources](https://www.affinity.studio/resources)
    *   [Press](https://www.canva.com/newsroom/news/all-new-affinity)

*   Help 

    *   [Help Center](https://www.affinity.studio/help)
    *   [Affinity V2](https://affinity.serif.com/v2/)
    *   [Affinity V2 account](https://store.serif.com/sign-in/)
    *   [April 2026 update](https://www.affinity.studio/blog/affinity-update-april-2026)
    *   [Release notes](https://www.affinity.studio/help/release-notes/)

*   Connect 

    *   [About](https://www.affinity.studio/about)
    *   [Careers](https://www.lifeatcanva.com/)
    *   [Contact Sales](https://www.canva.com/contact-sales/affinity/)

English (US)

*   [![Image 3: Instagram](https://content-management-files.canva.com/42a74cd5-ccc1-43f4-847b-cfee15b387d6/social_instagram.svg)](https://www.instagram.com/affinity/)
*   [![Image 4: Facebook](https://content-management-files.canva.com/c0ee217b-1e52-423a-95cb-6674227da28c/social_facebook.svg)](https://www.facebook.com/affinity)
*   [![Image 5: Threads](https://content-management-files.canva.com/5a46d885-129b-44bc-9bb5-91a1cd24519a/social_threads.svg)](https://www.threads.com/@affinity)
*   [![Image 6: Discord](https://content-management-files.canva.com/4c86f6ba-0400-48aa-b33b-d1ac6e06406f/social_discord.svg)](https://affin.link/dc)
*   [![Image 7: TikTok](https://content-management-files.canva.com/c38ff714-7599-4dc5-869a-7bc1553075e5/social_tiktok.svg)](https://www.tiktok.com/@weareaffinity)
*   [![Image 8: X](https://content-management-files.canva.com/1c162bcb-f637-4d23-ac3f-909ebf33f8c0/social_twitter.svg)](https://x.com/Affinity)
*   [![Image 9: Youtube](https://content-management-files.canva.com/719b9b23-2ce5-4dcf-8487-d81d4fc52d81/social_youtube.svg)](https://www.youtube.com/AffinitySuite)

*   [Privacy](https://www.canva.com/policies/privacy-policy/)
*   [Terms & Conditions](https://www.canva.com/policies/)
*   © 2026 All Rights Reserved, Affinity 

Show main menu

Suggestions will appear below the field as you type

Displaying 5 of 18 links. Use the See All button at the end of the list to show all topics.Displaying 5 of 7 links. Use the See All button at the end of the list to show all topics.Displaying 5 of 20 links. Use the See All button at the end of the list to show all topics.Displaying 5 of 24 links. Use the See All button at the end of the list to show all topics.Displaying 5 of 21 links. Use the See All button at the end of the list to show all topics.Displaying 5 of 25 links. Use the See All button at the end of the list to show all topics.Displaying 5 of 24 links. Use the See All button at the end of the list to show all topics.Displaying 3 of 3 links. Use the See All button at the end of the list to show all topics.Displaying 5 of 12 links. Use the See All button at the end of the list to show all topics.Displaying 5 of 82 links. Use the See All button at the end of the list to show all topics.

![Image 10: Affinity Logo](https://cdn-au.onetrust.com/logos/3dbea99f-abc0-4dbd-bcd7-8f6dfcaea28d/019a2649-863c-7345-9b60-9a618abba327/5c38b3e1-3c43-4eda-b904-88377c9b9c2e/full-logo.png)

## Manage Cookies

Cookies and similar technologies collect certain information about how you’re using our website. Some of them are essential, and without them this affinity.studio website might not work. But others are optional, and you get to choose whether we use them or not. 

 Hungry for more? 

[Read our full Cookie policy here.](https://www.canva.com/policies/cookies-policy/)

Allow All

#### Strictly Necessary Cookies

Always Active

These cookies are necessary for the website to function and cannot be switched off in our systems. They are usually only set in response to actions made by you which amount to a request for services, such as setting your privacy preferences, logging in or filling in forms. You can set your browser to block or alert you about these cookies, but some parts of the site will not then work.

Cookies Details

#### Performance Cookies

- [x] Performance Cookies 

These cookies allow us to count visits and traffic sources so we can measure and improve the performance of our site. They help us to know which pages are the most and least popular and see how visitors move around the site. All information these cookies collect is aggregated and therefore anonymous. If you do not allow these cookies we will not know when you have visited our site, and will not be able to monitor its performance.

Cookies Details

#### Targeting Cookies

- [x] Targeting Cookies 

These cookies may be set through our site by our advertising partners. They may be used by those companies to build a profile of your interests and show you relevant adverts on other sites. They do not store directly personal information, but are based on uniquely identifying your browser and internet device. If you do not allow these cookies, you will experience less targeted advertising.

Cookies Details

### Learn More

Clear
*   - [x] checkbox label label 

Apply Cancel

Consent Leg.Interest

- [x] checkbox label label

- [x] checkbox label label

- [x] checkbox label label

Reject All Confirm My Choices

[![Image 11: Powered by Onetrust](https://cdn-au.onetrust.com/logos/static/powered_by_logo.svg)](https://www.onetrust.com/solutions/consent-and-preferences/)
