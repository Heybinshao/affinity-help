---
title: "Adjustment layers - Affinity Help Center"
source: https://www.affinity.studio/help/layers-adjustment-layers/
slug: layers-adjustment-layers
fetched: 2026-08-06
---

# Adjustment layers - Affinity Help Center

> 官方来源：https://www.affinity.studio/help/layers-adjustment-layers/

[Skip to main content](https://www.affinity.studio/help/layers-adjustment-layers/#_r_0_)
...
Use adjustment layers to apply a non-destructive adjustment to a specific layer, group, object or an image on your page, as well as the entire page content.

[](https://www.affinity.studio/help/layers-adjustment-layers/#about-adjustment-layers)

## About adjustment layers

Adjustments include customizable settings alongside general layer options. Once applied, they can be identified on the Layers panel as both an adjustment and a specific adjustment type by using unique symbols.

![Image 4: Adjustment layer type](https://images.ctfassets.net/3p2fxa94bzao/46w37ue46iJeoGJbZo7Ttx/3205fe01526b5abe013c3c4819e43590/adjustment_layertype.png)

Curves adjustment layer indicating it is an adjustment (A) and a Curves adjustment type (B).

There may be times that you only want to apply an adjustment layer to either a single layer (e.g., a placed image) or a group of layers. This is easily achieved by [clipping](https://www.affinity.studio/help/layers-layer-clip/).

If you have a [pixel selection](https://www.affinity.studio/help/selections-selections-create/) in place when you add an adjustment layer, the area selected is automatically masked. If you have a vector object selected, then the adjustment layer is created as a child of the object layer. This behavior can be changed in Assistant Settings.

Adjustment layers also have mask layer properties. Areas of the layer can be revealed or hidden in the same way as with a [mask layer](https://www.affinity.studio/help/layers-layer-masks/).

Available adjustments are listed in [Applying adjustments](https://www.affinity.studio/help/adjustments-adjustment-applying/).

Depending on if a layer is selected or not, the created adjustment layer is placed differently according the **Add adjustment layer to selection** option in Assistant Settings:

*   With the **Add Adjustment layer to parent layer** option set (default), when an item is selected, the adjustment layer is applied directly above the item's layer. With no selection in place, the adjustment is applied at the top of the layer stack.
*   With the **Add Adjustment layer to selection** option set, when an item is selected, the adjustment layer is applied to just that item, i.e. it is [clipped](https://www.affinity.studio/help/layers-layer-clip/). With no selection in place, the adjustment is applied at the top of the layer stack.

[](https://www.affinity.studio/help/layers-adjustment-layers/#see-also:)
